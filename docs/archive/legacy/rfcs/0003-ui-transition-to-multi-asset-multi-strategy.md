...
### 6.1 패널 크기 조절(사용자 제안 반영)

- 목표: 고정 grid 비율이 아니라, 운영자가 실행 중 패널 비율을 늘리거나 줄일 수 있어야 한다.
- 기본 동작:
  - `Ctrl+D`: 현재 포커스 패널 비율 +10% (인접 패널에서 균등 차감)
  - `Ctrl+Shift+D`: 현재 포커스 패널 비율 -10%

### 6.2 기술 가능성 및 제약

- 기술적으로 가능:
  - `ratatui` + `crossterm` 이벤트 루프에서 패널 weight를 상태로 들고 있고, 키 이벤트마다 weight를 갱신해 재렌더하면 된다.
- 제약:
  - `Ctrl+Shift+D`는 많은 터미널에서 `Ctrl+D`와 구분되지 않거나, 터미널 자체 단축키(split pane)에 의해 선점될 수 있다.
  - 따라서 `Ctrl+Shift+D`는 **best-effort** 지원으로 두고, 실패 시 확실한 폴백 키를 제공해야 한다.
...
- 기본 폴백:
  ...
  - `[`: 현재 패널 축소
- 고급 모드:
  - `z` 입력 시 `Resize Mode` 진입 후 방향키(`←→↑↓`)로 인접 패널 비율 조절, `Enter` 저장, `Esc` 취소
- `config/default.toml`에 키맵 오버라이드 허용:
  - `ui.keymap.resize_grow`
  - `ui.keymap.resize_shrink`
  - `ui.keymap.resize_mode`
...
- 지원 터미널에서 `Ctrl+D` 리사이즈 동작, 미지원 터미널에서도 폴백 키(`[`, `]`)로 동일 기능 수행
