...
fn ensemble_vote_emits_actionable_signal() {
    ...
    for p in [120.0, 118.0, 116.0, 114.0, 112.0, 110.0, 108.0, 106.0, 104.0, 102.0] {
        let _ = strategy.on_tick(&tick(p));
    }

    for p in [
        103.0, 105.0, 107.0, 109.0, 111.0, 113.0, 115.0, 116.0, 120.0, 118.0, 114.0, 110.0,
        106.0, 102.0, 99.0, 96.0, 93.0,
    ] {
