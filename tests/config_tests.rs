...
"#;
    ...
    assert!(config.alpha.enabled);
    assert_eq!(config.alpha.mode, "shadow");
    assert_eq!(config.alpha.lookback_trades, 200);
    assert!((config.alpha.forward_p_win - 0.5).abs() < f64::EPSILON);
    assert!((config.alpha.forward_target_rr - 1.5).abs() < f64::EPSILON);
    assert!((config.alpha.predictor_mu - 0.0).abs() < f64::EPSILON);
    assert!((config.alpha.predictor_sigma_spot - 0.01).abs() < f64::EPSILON);
    assert!((config.alpha.predictor_sigma_futures - 0.015).abs() < f64::EPSILON);
    assert!((config.alpha.futures_multiplier - 1.0).abs() < f64::EPSILON);
