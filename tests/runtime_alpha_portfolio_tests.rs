...
fn alpha_portfolio_decision_maps_to_execution_intent() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 42, true, 0.5, 100.0, trend_up_regime(1.0));
...
fn intent_effective_signal_uses_target_ratio_first() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 100, true, 0.3, 50.0, trend_up_regime(1.0));
...
fn sizing_scales_target_ratio_by_strength_bucket() {
    let low =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.05, 100.0, trend_up_regime(1.0));
    let high =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.9, 100.0, trend_up_regime(1.0));
...
fn effective_signal_holds_when_delta_within_threshold() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.5, 100.0, trend_up_regime(1.0));
...
fn alpha_only_decision_enters_when_alpha_positive() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.4, 100.0, trend_up_regime(1.0));
...
fn alpha_only_decision_exits_when_alpha_non_positive_and_open() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, false, 0.0, 100.0, trend_up_regime(1.0));
...
fn trend_down_regime_blocks_new_entries() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.8, 100.0, down_regime(1.0));
...
fn unknown_regime_blocks_new_entries() {
    let decision =
        decide_portfolio_action_from_alpha("BTCUSDT", 1, true, 0.8, 100.0, unknown_regime(0.9));
    assert_eq!(decision.execution_signal, Signal::Hold);
    assert_eq!(decision.target_position_ratio, 0.0);
}
