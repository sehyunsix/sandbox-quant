...
impl MaCrossover {
    pub fn new(fast_period: usize, slow_period: usize, order_qty: f64, min_ticks: u64) -> Self {
        assert!(
...
mod tests {
    ...
    fn insufficient_data_returns_hold() {
        ...
        let mut strat = MaCrossover::new(2, 3, 0.001, 0);
    ...
    fn buy_signal_on_bullish_crossover() {
        ...
        let mut strat = MaCrossover::new(2, 4, 0.001, 0);
        ...
        assert!(
            matches!(sig, Signal::Buy { qty } if (qty - 0.001).abs() < f64::EPSILON),
    ...
    fn cooldown_prevents_rapid_signals() {
        let mut strat = MaCrossover::new(2, 4, 0.001, 100); // 100-tick cooldown
    ...
    fn deterministic_output() {
        ...
        let run = |prices: &[f64]| -> Vec<Signal> {
            let mut strat = MaCrossover::new(5, 15, 0.001, 0);
